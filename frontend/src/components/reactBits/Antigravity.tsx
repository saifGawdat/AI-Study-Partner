...
// Simple seeded random to satisfy purity requirements
...
const AntigravityInner: React.FC<AntigravityProps> = ({
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const s = i * 100;
      const t = seededRandom(s + 1) * 100;
      const factor = 20 + seededRandom(s + 2) * 100;
      const speed = 0.01 + seededRandom(s + 3) / 200;
      const xFactor = -50 + seededRandom(s + 4) * 100;
      const yFactor = -50 + seededRandom(s + 5) * 100;
      const zFactor = -50 + seededRandom(s + 6) * 100;

      const x = (seededRandom(s + 7) - 0.5) * width;
      const y = (seededRandom(s + 8) - 0.5) * height;
      const z = (seededRandom(s + 9) - 0.5) * 20;

      const randomRadiusOffset = (seededRandom(s + 10) - 0.5) * 2;
